...
public sealed record UpdateOperationCommand(Guid MasterId, Guid OperationId, int Sequence, string Name, string Instructions, WorkRole[] PrimaryBuyoffRoles, WorkRole[] SecondaryBuyoffRoles);
...
public sealed class AddOperationHandler(IEngineeringMasterRepository repository, IUnitOfWork uow)
{
    public async Task<Result<OperationDto>> HandleAsync(AddOperationCommand cmd, CancellationToken ct)
    {
        ...
        return new OperationDto(op.Id.Value, op.Sequence, op.Name, op.Instructions,
            op.PrimaryBuyoffRoles.Select(r => r.ToString()).ToList(),
            op.SecondaryBuyoffRoles.Select(r => r.ToString()).ToList(), [], []);
    }
}
...
public sealed class UpdateOperationHandler(IEngineeringMasterRepository repository, IUnitOfWork uow)
{
    public async Task<Result> HandleAsync(UpdateOperationCommand cmd, CancellationToken ct)
    {
        ...
        var result = master.UpdateOperation(new OperationId(cmd.OperationId), cmd.Sequence, cmd.Name, cmd.Instructions, cmd.PrimaryBuyoffRoles, cmd.SecondaryBuyoffRoles);
