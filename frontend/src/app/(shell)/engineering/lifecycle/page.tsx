...
import { partsApi, type PartSummary, PART_TYPE_LABELS } from '@/modules/engineering/parts/api';
      ...
      {loadError ? (
        ...
        <div className="surface rounded-sm px-6 py-16 text-center">
          ...
          <div className="mt-1 text-xs text-ink-400">
            Create parts in the{' '}
            <Link href="/engineering/parts" className="text-accent hover:underline">
              Part Portal
            </Link>{' '}
            ...
            <tbody className="divide-y divide-ink-700/60">
              {parts.map((p) => (
                <tr key={p.id} className="hover:bg-ink-800/40 transition-colors">
                  <Td>
                    <Link
                      href={`/engineering/lifecycle/${encodeURIComponent(p.partNumber)}`}
